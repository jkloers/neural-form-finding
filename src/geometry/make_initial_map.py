...
def _project_to_boundary(vertices, boundary_points):
    ...
    # Maximum distance to the edge of the tessellation bounding box (for normalization)
    # Assuming a rectangular box [w, h]
    w, h = (p_max[0] - p_min[0])/2.0, (p_max[1] - p_min[1])/2.0
    # Distance to the edge of a rectangle with half-sides w, h at angle theta
    # r = 1 / max(|cos/w|, |sin/h|)
...
def compute_initial_map(
    ...
    """Remaps the vertices to the target shape."""
    ...
    return Tessellation(
        ...
        border_edges=tessellation.border_edges,
    )
