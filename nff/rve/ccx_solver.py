...
def _write_inp(path, xyz, conn, arcA, arcB, pivot, mat, angle, elastic_only, n_frames, hyp):
    n_ref, n_rot = len(xyz) + 1, len(xyz) + 2
    rigid = bool(rigid_drive or free_dofs)
    L = ["*NODE"]
    for i, (x, y, z) in enumerate(xyz, start=1):
        L.append(f"{i}, {x:.6e}, {y:.6e}, {z:.6e}")
    if rigid:
        # both driver nodes sit ON the pivot, so the reference node's translations ARE (a, s) and
        # the rotation node's DOF 3 IS theta -- the oracle's kinematics become literal DOF
        z_mid = 0.5 * float(xyz[:, 2].min() + xyz[:, 2].max())
        L.append(f"{n_ref}, {pivot[0]:.6e}, {pivot[1]:.6e}, {z_mid:.6e}")
        L.append(f"{n_rot}, {pivot[0]:.6e}, {pivot[1]:.6e}, {z_mid:.6e}")
...
def _write_deck(path, xyz, conn, arcA, arcB, pivot, mat, states, elastic_only, field_every,
                ...
                field_freq=1000, el_fields=None, free_dofs=(), rigid_drive=False):
    ...
    ``free_dofs``: subset of ``{"a", "s"}`` left UNPRESCRIBED. Driving the arc through a
    ``*RIGID BODY`` reference node placed at the pivot makes (a, s, theta) literal DOF, so a
    component can simply be left free -- zero applied force -- and the solver then finds the opening
    that MINIMISES energy at each imposed rotation. That is a least-energy route generated by the
    physics, instead of one inherited from the ROM (whose linear k_rot under-prices the first few
    degrees of rotation and so produces rotation-first paths). Free DOF also make the reactions
    exact: RF at the reference node IS (F_a, F_s) and at the rotation node IS M_theta, with no
    lever-arm summation. Empty ⟹ all three prescribed, via the same rigid body.
    """
    ...
    if rigid:
        L.append(f"*RIGID BODY, NSET=ARCB, REF NODE={n_ref}, ROT NODE={n_rot}")
    ...
    for k, (a, s, th) in enumerate(states):
        ...
        if rigid:
            L.append(f"{n_rot}, 1, 2, 0.0")                    # no out-of-plane rotation
            L.append(f"{n_rot}, 3, 3, {th:.6e}")               # the imposed fold
            if not arcB_uz_free:
                L.append(f"{n_ref}, 3, 3, 0.0")
            if "a" not in free_dofs:
                L.append(f"{n_ref}, 1, 1, {a:.6e}")
            if "s" not in free_dofs:
                L.append(f"{n_ref}, 2, 2, {s:.6e}")
        else:
        ...
        if rigid:                                              # exact generalized reactions + the
            L.append("*NODE PRINT, NSET=NDRIVE\nRF, U")        # solver-chosen (a, s)
...
def prepare_job(p, angle_deg=60.0, n_steps=15, pivot=None, material=STEEL, imp_amp=None,
    ...
    _write_deck(job + ".inp", xyz, conn, arcA, arcB, pivot, mat, states, elastic_only,
                ...
                free_dofs=free_dofs, rigid_drive=rigid_drive)
    return dict(job=job, workdir=workdir, xyz=xyz, conn=conn, arcA=arcA, arcB=arcB, pivot=pivot,
                ...
                free_dofs=tuple(free_dofs), rigid_drive=bool(rigid_drive or free_dofs),
...
def solve_job(meta, ncpus=1, timeout=1800, eps_f=None, fracture_margin=1.1, poll=4.0,
    ...
    if eps_f is None:
        r = subprocess.run([ccx_bin, "hinge"], cwd=meta["workdir"], capture_output=True,
                           text=True, timeout=timeout, env=env)
        meta["stop_reason"] = "completed" if "Job finished" in (r.stdout or "") else "diverged"
        return r
    ...
    with open(logpath, "w") as logf:                             # file, not PIPE: no buffer deadlock
        ...
        while True:
            ...
            if time.time() - t0 > timeout:
                reason = "timeout"; proc.terminate(); break
            try:                                                 # peek the partial .frd for fracture
                ...
                if frames and peak_peeq(frames[-1], xyz, conn, w_lig) >= fracture_margin * eps_f:
                    reason = "fractured"; proc.terminate(); break
            except Exception:
                pass
    ...
    with open(logpath) as f:
        out = f.read()
    if reason == "diverged" and "Job finished" in out:
        reason = "completed"
...
def parse_job(meta, stdout=""):
    ...
    return dict(ok=ok, stdout=stdout[-1500:], stop_reason=meta.get("stop_reason", "unknown"),
                theta_deg=theta_deg, a=a_imp, s=s_imp,
